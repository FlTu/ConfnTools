(require 'semantic)
(require 'sr-speedbar)
;; Raccourcis clavier personnels M- & M-S- & M-C- & M-C-S
(defvar fulw 209.)
(defvar fulh 46.)
...
;;( define-key dired-mode-map (kbd "<return>") 'dired-find-alternate-file)       ;; M-x redirection
  ...
  ( global-set-key (kbd "C-M-c") 'uncomment-region)

  ;; Rechercher / remplacer caractères
  ( global-set-key (kbd "M-q"  )     'query-replace)            ;;trouver-remplacer (default : M-%) 
  ( global-set-key (kbd "M-G"  )     'grep-find)                ;;trouver dans les fichiers [...] 
  ( global-set-key (kbd "M-S")     'search-forward-regex)     ;;trouver dans les fichiers [...] 
  ( global-set-key (kbd "M-R"  )     'search-backward-regex)    ;;trouver dans les fichiers [...] 

  ;; Parcourir / Manipuler Fichiers
  ( global-set-key (kbd "C-M-f"    )   'find-lisp-find-dired)     ;;recherche de fichier
  ( global-set-key (kbd "M-H")   'ff-find-other-file)       ;;switch headers/sources
  ( global-set-key (kbd "C-M-H"  )   'ff-find-related-file)     ;;switch headers/sources
  ( global-set-key (kbd "M-L"    )   'windmove-right)           ;;passer à la WINDOW à droite
  ( global-set-key (kbd "M-J"    )   'windmove-left)            ;;passer à la WINDOW à gauche
  ( global-set-key (kbd "M-I"    )   'windmove-up)           ;;passer à la WINDOW à droite
  ( global-set-key (kbd "M-K"    )   'windmove-down)            ;;passer à la WINDOW en bas
;;  ( global-set-key (kbd "C-M-<"  )   'previous-buffer)          ;;WINDOW précédente
;;  ( global-set-key (kbd "C-M->"  )   'next-buffer)              ;;WINDOW suivante
  ( global-set-key (kbd "M-µ"    )   'switch-to-buffer)         ;;Afficher le fichier [...]
  ...
  ;; Environnement 
  ( global-set-key (kbd "<backtab>") 'dired-default-directory-on-left)  ;;Ouvre/ferme le panneau latéral affichant le répertoire courant
  ( global-set-key (kbd "M-T")     'term-at-bottom)                   ;;Ouvre/ferme le panneau terminal
  ( global-set-key (kbd "M-s")     'sr-speedbar-toggle)                   ;;Ouvre/ferme la speedbar integree
  ( global-set-key (kbd "M-C-b")     'buffer-menu-at-bottom-left)       ;;Ouvre/ferme le panneau terminal
